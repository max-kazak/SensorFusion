                      ...
                      vector<cv::DMatch> &matches, string descriptorType, string matcherType, string selectorType)
{
    ...
    // configure matcher
    bool crossCheck = selectorType.compare("SEL_KNN") == 0 ? false : true;
...
char* formatPath(char const *str_template, char const *param)
{
	size_t needed = snprintf(NULL, 0, str_template, param) + 1;
	char* buffer = (char*) malloc(sizeof(char) * needed);
	sprintf(buffer, str_template, param);
	return buffer;
}
...
int main()
{
    ...
    const char* dataset = "BRISK_small";

    vector<cv::KeyPoint> kptsSource, kptsRef; 
    readKeypoints(formatPath("../dat/C35A5_KptsSource_%s.dat", dataset), kptsSource);
    readKeypoints(formatPath("../dat/C35A5_KptsRef_%s.dat", dataset), kptsRef);

    cv::Mat descSource, descRef; 
    readDescriptors(formatPath("../dat/C35A5_DescSource_%s.dat", dataset), descSource);
    readDescriptors(formatPath("../dat/C35A5_DescRef_%s.dat", dataset), descRef);
